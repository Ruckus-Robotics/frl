...
TEST_F(RigidBodyTransformTest, testApplyTranslation)
{
    ...
    for (int i = 0; i < nTests; i++)
    {
        transform = GeometryUtilitiesTestHelper::createRandomTransformationMatrix<double>();
        vec1 = GeometryUtilitiesTestHelper::createRandomVector3<double>();
        ...
        EXPECT_TRUE(GeometryUtilitiesTestHelper::areVector3sEpsilonEqual<double>(check, newTrans, 1e-5));;
    }
}
